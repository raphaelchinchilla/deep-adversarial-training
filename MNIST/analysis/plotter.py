...
def plot_filters(args, net, fig_name = ''):
	...
	for i, ax in enumerate(axes.flat):
		ax.set_axis_off()
		im = ax.imshow(net.conv1.weight.view(-1,net.conv1.weight.shape[-1], net.conv1.weight.shape[-1]).cpu().detach().numpy()[i], cmap='viridis', vmin = net.conv1.weight.min(), vmax = net.conv1.weight.max())
	...
	for i, ax in enumerate(axes.flat):
		ax.set_axis_off()
		im = ax.imshow(net.conv2.weight.view(-1,net.conv2.weight.shape[-1], net.conv2.weight.shape[-1]).cpu().detach().numpy()[i], cmap='viridis', vmin = net.conv2.weight.min(), vmax = net.conv2.weight.max())
		ax.set_title(f'Filter {i}')
		plt.axis("off")
...
def plot_features(args, net, test_loader, device, data_params, attack_params, fig_name):
	...
	perturbs = PGD(net, img, lbl, device, data_params = data_params, attack_params = attack_params)
	img += perturbs
	img = torch.clamp(img, 0, 1)

	output = fmodel(img)
	adversarial_features = output.view(-1, output.shape[-1], output.shape[-1]).cpu().detach().numpy()
	out = net(img)
	pred = out.argmax(dim=1, keepdim=True)
	...
	for i, ax in enumerate(axes.flat):
		ax.set_axis_off()
		im = ax.imshow(adversarial_features[i], cmap='viridis', vmin = output.min(), vmax = output.max())
		ax.set_title(f'Feature {i}')
		plt.axis("off")
	...
	for i, ax in enumerate(axes.flat):
		ax.set_axis_off()
		im = ax.imshow((normalize(adversarial_features) - normalize(clean_features))[i], cmap='viridis', vmin = (normalize(adversarial_features) - normalize(clean_features)).min(), vmax = (normalize(adversarial_features) - normalize(clean_features)).max())
		ax.set_title(f'Feature {i}')
		plt.axis("off")
	...
	small_value = 0.00001
	print(np.sum(np.abs(normalize(adversarial_features) - normalize(clean_features)), axis = (1,2))/(np.sum(normalize(clean_features), axis =(1,2)) + small_value))
