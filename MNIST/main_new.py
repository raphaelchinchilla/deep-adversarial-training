...
def main():
    ...
    optimizer = optim.SGD(
        model.w + model.b, lr=args.learning_rate, momentum=args.momentum)

    print(model)

    weight_momen = list()
    for k in range(len(num_neurons)):
        weight_momen.append(torch.zeros(model.w[k].size()))
    ...
    for e in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            lamb = 0.1  # budget for perturbation
            mu = 0.9  # adverserial step
            model.d[0] = math.sqrt(
                lamb)*torch.randn(data.size(0), num_neurons[0]).to(device)
            model.d[1] = math.sqrt(
                lamb)*torch.randn(data.size(0), num_neurons[1]).to(device)
            with torch.no_grad():
                output = model(data)
            ...
            for count in range(10):
                ...
                perturb.view(-1, 1).repeat(1, data.size(0)
                                           ).backward(torch.eye(data.size(0)).to(device))
